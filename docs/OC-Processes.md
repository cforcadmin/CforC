...
| Προβολή Δεικτών | ✅ | ✅ | ✅ | ✅ |
| **Διορθώσεις / Προτάσεις** | — | ✅ μόνο IT | — | — |

Η **διπλή υπογραφή** στο κλείσιμο του μήνα είναι σκόπιμη: αυτός που ελέγχει τα νούμερα δεν είναι αυτός που τα στέλνει έξω.

**Διορθώσεις / Προτάσεις (μόνο IT).** Ένατη ενότητα, ορατή μόνο όταν η ενεργή θέση είναι IT. Ένα κουτί για κάθε σελίδα του OC (Επισκόπηση … Ρυθμίσεις), καθένα με **δικό του πίνακα εκκρεμοτήτων** — ο ίδιος μηχανισμός με Διαχείριση → Εκκρεμότητες (ανάδοχος, προτεραιότητα, προθεσμία, κατάσταση, πίνακας/λίστα). Οι πίνακες είναι `oc-task-board` με slug `it-corrections-<σελίδα>`, δημιουργούνται μόνοι τους την πρώτη φορά, **δεν εμφανίζονται** στον επιλογέα πινάκων της Διαχείρισης, και το API απορρίπτει κάθε ανάγνωση/εγγραφή τους από άλλη θέση (403) — όχι μόνο η οθόνη. Καμία αλλαγή σχήματος στο Strapi.
...
**Το κλείσιμο γίνεται σε δύο στάδια:** ο/η Ταμίας πατά «Έτοιμο προς αποστολή» — τη στιγμή εκείνη φεύγει email «Ο μήνας είναι έτοιμος» στη θυρίδα της Διαχείρισης (hello@, από finance@) με σύνοψη εσόδων/εξόδων/ισοζυγίου και πλήθος εκ των υστέρων, ώστε η δεύτερη υπογραφή να καλείται και όχι να το ανακαλύπτει τυχαία — και μόνο τότε ενεργοποιείται για τη Γραμματεία το «Αποστολή στο λογιστήριο». Φεύγει email με **CSV** που έχει τμήμα Α, τμήμα Β, (Γ αν υπάρχει), σύνολα και ισοζύγιο, και με **απευθείας συνδέσμους** στους φακέλους Drive του μήνα (Έξοδα και Έσοδα — όσους επιστρέψει το web app). Παραλήπτης το `ACCOUNTANT_EMAIL`, με κοινοποίηση finance@ και hello@.
